...
function showLocation(locURL) {
    ...
    $.ajax({
        ...
        for (var i = 0; i < result.length; i++) {
            var showtimes = result[i].showtimes;
            console.log(i);
            
            for(var j=0; j<showtimes.length; j++){
            
            
            var locationName = showtimes[j].theatre.name;
            movieTime.push(showtimes[j].dateTime);
            var name1 = result[i].title;

            locMovies = {
                "title" : result[i].title,
                "locationName" : showtimes[j].theatre.name,
                "movieTime" : movieTime
            } 
            console.log(locationName);
            console.log(movieTime);
            console.log(name1);
            console.log(locMovies);
        ...
        $("#theatre").on('click', function () {
            var newDiv = $("<div>");
            newDiv.addClass("output-section");
            newDiv.attr("my-output");
            $("#theatre").prepend(newDiv);
        })
